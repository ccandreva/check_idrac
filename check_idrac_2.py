...
def is_number(parameters, x):  # input parameters and how many need to parse
    if not parameters:
        parameters = []
        for i in range(0, x):
            # auto fills 'none' if parameter not set
            y = parameters.append('none')
        return parameters
    else:
        ...
        for r in range(0, x):
            try:
                ...
                # convert none number (float) into string
                return_parameters.append('none')
            except IndexError:
                # auto fills 'none' if parameter not set
                return_parameters.append('none')
...
def cli_reader():
    ...
    optp.add_option('-v', help='specifies SNMP version to use',
                    dest='version', metavar='2c|3')
    optp.add_option('-u', help='set security name',
                    dest='sec_name', metavar='USER-NAME')
    optp.add_option('-a', help='set authentication protocol (MD5|SHA)',
                    dest='auth_protocol', metavar='PROTOCOL')
    optp.add_option('-A', help='set authentication protocol pass phrase',
                    dest='auth_pass', metavar='PASSPHRASE')
    optp.add_option('-x', help='set privacy protocol (DES|AES)',
                    dest='pri_protocol', metavar='PROTOCOL')
    optp.add_option('-X', help='set privacy protocol pass phrase',
                    dest='pri_pass', metavar='PASSPHRASE')
    optp.add_option('-l', help='set security level (noAuthNoPriv|authNoPriv|authPriv)',
                    dest='level', metavar='LEVEL')
    optp.add_option('-c', help='SNMPv2 community string',
                    dest='community', type='string', metavar='COMMUNITY')
    optp.add_option(
        '-f', help='configuration file. COMMAND LINE WILL OVERWRITE CONFIGURATION FILE PARAMETERS', dest='cfg', metavar='FILE')
    optp.add_option(
        '-m', help='specific MIB file. Default is load all MIBs', dest='mib', metavar='FILE')
    optp.add_option('-n', '--no-alert', help='always return with exit code 0',
                    action='store_true', dest='no_alert')
    optp.add_option('-w', help='hardware to check. If no hardware specified, all will be listed: DISK, VDISK, FAN, SENSOR, CPU, PS, PU, MEM, BATTERY',
                    dest='hardware', metavar='FAN|FAN#1|MEM')
    optp.add_option('-p', help='enable performance data',
                    dest='perf', action='store_true')
    optp.add_option('--fan-warn', help='FAN rpm warning thresholds',
                    dest='fan_warn', metavar='MIN,MAX')
    optp.add_option('--fan-crit', help='FAN rpm critical thresholds',
                    dest='fan_crit', metavar='MIN,MAX')
    optp.add_option('--temp-warn', help='TEMPERATURE warning thresholds',
                    dest='temp_warn', metavar='MIN,MAX')
    optp.add_option('--temp-crit', help='TEMPERATURE critical thresholds',
                    dest='temp_crit', metavar='MIN,MAX')
    optp.add_option('--volt-warn', help='output VOLTAGE warning thresholds',
                    dest='volt_warn', metavar='MIN,MAX')
    optp.add_option('--volt-crit', help='output VOLTAGE critical thresholds',
                    dest='volt_crit', metavar='MIN,MAX')
    optp.add_option('--cur-warn', help='output CURRENT warning thresholds',
                    dest='cur_warn', metavar='MIN,MAX')
    optp.add_option('--cur-crit', help='output CURRENT critical thresholds',
                    dest='cur_crit', metavar='MIN,MAX')
    optp.add_option('--wat-warn', help='output WATT warning thresholds',
                    dest='wat_warn', metavar='MIN,MAX')
    optp.add_option('--wat-crit', help='output WATT critical thresholds',
                    dest='wat_crit', metavar='MIN,MAX')
    optp.add_option('--pwr-alert', help='power consumption',
                    dest='consume', metavar='WARN,CRIT')
    optp.add_option('--vdisk-bad', help='number of bad block',
                    dest='vdisk', metavar='WARN,CRIT')
    optp.add_option(
        '--ok', help='ok|online|on|spunup|full|ready|enabled|presence', dest='ok', metavar='STATES')
    optp.add_option('--warn', help='$ALL$', dest='warn', metavar='STATES')
    optp.add_option('--crit', help='critical|nonRecoverable|fail',
                    dest='crit', metavar='STATES')
    optp.add_option('--mem-modules', help='number of installed memory modules',
                    dest='mem_modules', metavar='NUMBER')
    ...
    if opts.host is None:
        ...
        if opts.version:
            conf['snmp_version'] = opts.version
        if opts.community:
            conf['snmp_community'] = opts.community
        if opts.sec_name:
            conf['snmp_security_name'] = opts.sec_name
        if opts.level:
            conf['snmp_security_level'] = opts.level
        if opts.auth_protocol:
            conf['snmp_authentication_protocol'] = opts.auth_protocol
        if opts.auth_pass:
            conf['snmp_authentication_password'] = opts.auth_pass
        if opts.pri_protocol:
            conf['snmp_privacy_protocol'] = opts.pri_protocol
        if opts.pri_pass:
            conf['snmp_privacy_password'] = opts.pri_pass
        if opts.hardware:
            conf['hardware'] = opts.hardware
        if opts.mib:
            conf['mib'] = opts.mib
        else:
            conf['mib'] = 'ALL'  # if not set, then search all mibs
        if opts.cfg:
            conf['file'] = opts.cfg
        if opts.ok:
            conf['state_ok'] = opts.ok
        if opts.warn:
            conf['state_warn'] = opts.warn
        if opts.crit:
            conf['state_crit'] = opts.crit
        if opts.no_alert is True:
            conf['alert'] = False
        if opts.perf is True:
            conf['perf'] = True
        if opts.mem_modules:
            conf['mem_modules'] = opts.mem_modules
        # parse fan threshold
        conf['fan_thresholds'] = []
...
def config_reader():
    if not os.path.isfile(conf['file']):
        ...
        except configparser.ParsingError as err_parsing:
            print('invalid configuration syntax: %s' % err_parsing.errors)
        except IOError as err_read_file:
            print(err_read_file)
        else:
            for key in list(conf_file.keys()):
                try:
                    conf_file[key] = reader.get(
                        key.split('_')[0], key.split('_', 1)[1])
...
def config_verify():  # check if configurations are properly set
    ...
    if conf['file'] is not None:
        for threshold in ['fan_thresholds', 'voltage_thresholds', 'current_thresholds', 'watt_thresholds',
            ...
            for i in range(0, len(conf[threshold])):
                if conf[threshold][i] == 'none':
                    conf[threshold][i] = conf_file[threshold][i]
        for parameter in ['snmp_version', 'snmp_community', 'snmp_security_name', 'snmp_authentication_protocol',
                          ...
                          'snmp_security_level']:
            if conf[parameter] is None:
                conf[parameter] = conf_file[parameter]
        if conf['state_ok'] is None:
            conf['state_ok'] = conf_file['state_ok']
        if conf['state_warn'] is None:
            conf['state_warn'] = conf_file['state_warn']
        if conf['state_crit'] is None:
            conf['state_crit'] = conf_file['state_crit']
    # verify snmp
    ...
    if conf['state_ok'] is None:
        conf['state_ok'] = '0|ok|online|on|spunup|full|ready|enabled|presence'
    if conf['state_warn'] is None:
        conf['state_warn'] = '$ALL$'
    if conf['state_crit'] is None:
        ...
        if status != 0:
            ...
            if self.order is None:
                if self.hardware[2] == 'PS':
                    _, __ = run(snmp_cli.replace(
                        'powerSupplyTable', self.hardware[1][0]))
                    output += '\n' + __
                    _, __ = run(snmp_cli.replace(
                        'powerSupplyTable', self.hardware[1][1]))
                    output += '\n' + __
                elif self.hardware[2] == 'PU':
                    _, __ = run(snmp_cli.replace(
                        'powerUnitTable', self.hardware[1][0]))
                    output += '\n' + __
                    _, __ = run(snmp_cli.replace(
                        'powerUnitTable', self.hardware[1][1]))
                    output += '\n' + __
            return output.split('\n')
                ...
                if type(stat) == int:
                    if conf['state_warn'] == '$ALL$':
                        ...
                        if re.match(conf['state_ok'], tmp[key][stat], re.IGNORECASE):
                            ...
                            # more useful with check_mk frontend
                            tmp[key][stat] += '(!!)'
                            alert = 2
                        else:
                            tmp[key][stat] += '(!)'
                    ...
                    if self.hardware[2] == 'Fan':
                        ...
                        if stat_t == 6:
                            tmp[key][stat_t] = float(
                                tmp[key][stat_t].strip('(!)'))/1000
    ...
    def main(self):
        ...
        if self.order is None:
            ...
            if self.hardware[2] == 'PDisk' or self.hardware[2] == 'VDisk':
                oids = self.hardware[3].replace(
                    '|', ' ').replace('\.', '.%d' % self.order)
                snmp_data = self.get_snmp(oids)
            else:
                oids = self.hardware[3].replace(
                    '|', ' ').replace('\.', '.1.%d' % self.order)
                if self.hardware[2] == 'PS':
        ...
        if self.hardware[2] == 'PDisk':
            # raise alert on these value. Mapped with all_hardware as list order.
            value_on_alert = [3, 7, 9]
            ...
            for hw in list(hw_dict.values()):
                ...
                output.append(tmp % (self.hardware[2], hw[0], hw[1].split()[-1].replace('"', ''),
                                     hw_5, hw[3], hw[9], hw[6].replace(
                                         'HotSpare', '').replace('notASpare', 'no'),
            ...
            for hw in list(hw_dict.values()):
                ...
                if self.perf is True and hw[3].split('(')[0]:
                    perf_data = ' | FAN%s_RPM=%s;;;%s;%s' % (hw[0], hw[3].split(
                        '(')[0], conf['fan_thresholds'][2], conf['fan_thresholds'][3])
                    perf_data = perf_data.replace('none', '')
                else:
                    perf_data = ''
                output.append(tmp % (hw[4].replace('"', '').replace(
                    ' RPM', ''), hw[3], hw[1], hw[2], perf_data))
        elif self.hardware[2] == 'Sensor':
            # int type for status check, str type for range check
            value_on_alert = [1, 2, '3']
            if self.alert is True:
                hw_dict, exit_code = self.raise_alert(hw_dict, value_on_alert)
                tmp = '%s: %s C %s/%s%s'
            ...
            for hw in list(hw_dict.values()):
                ...
                if self.perf is True and hw[3] and hw[3] != '(N/A)':
                    ...
                    perf_data = ''
                if self.alert is True:
                    output.append(tmp % (hw[4].replace(
                        '"', ''), hw[3], hw[1], hw[2], perf_data))
                else:
                    if hw[3] == '(N/A)':
                        hw_3 = hw[3]
                    else:
            ...
            for hw in list(hw_dict.values()):
                ...
                if self.perf is True and hw[4].split('(')[0]:
                    perf_data = ' | PwrConsumption=%s;;;;' % (
                        hw[4].split('(')[0])
                    perf_data = perf_data.replace('none', '')
                else:
                    perf_data = ''
                output.append(
                    tmp % (self.hardware[2], hw[0], hw[1], hw[3], hw[2], hw[4], perf_data))
        elif self.hardware[2] == 'PS':
            ...
            for hw in list(hw_dict.values()):
                ...
                if self.alert is True:
                    if hw[4] == '(N/A)':
                        ...
                        hw_4 = float(hw[4])/10
                    output.append(tmp % (
                        self.hardware[2], hw[0], hw[1], hw[3], hw[6], hw[5], hw_4, hw[2], perf_data))
                else:
                    if hw[4] == '(N/A)':
                        hw_4 = hw[4]
                    else:
                    ...
                    if hw[2] == '(N/A)':
                        ...
                        hw_2 = float(hw[2])/10
                    output.append(
                        tmp % (self.hardware[2], hw[0], hw[1], hw[3], hw_6, hw_5, hw_4, hw_2))
            ...
            for hw in list(hw_dict.values()):
                ...
                output.append(tmp % (self.hardware[2], hw[0], hw[4].replace('.', ' ').replace('"', ''),
                                     hw_5, hw[6].split(
                )[-1], hw[1], hw[2], hw[3].replace('deviceTypeIs', ''),
            ...
            if conf['mem_modules'] and int(conf['mem_modules']) != len(hw_dict):
                ...
                if detect_diff <= -1:
                    output.append(
                        self.hardware[2] + ': Number of memory modules not correct!')

                elif detect_diff == 1:
                    output.append(
                        self.hardware[2] + ': 1 memory module not detected!!!')

                elif detect_diff >= 1:
                    output.append(
                        self.hardware[2] + ': ' + str(detect_diff) + ' memory modules not detected!!!')
...
if __name__ == '__main__':
    cli_reader()
    if conf['file'] is not None:
        for key in list(conf.keys()):  # trigger config_reader if there is None configuration
            if conf[key] is None:
                config_reader()
                break
    config_verify()
    if conf['hardware'] is None:  # check all hardware
        exit_code = 0
        for key in list(all_hardware.keys()):
        ...
        for key in list(all_hardware.keys()):
            if conf['hardware'][0] != key:
                not_found += 1
            if not_found == len(list(all_hardware.keys())):
